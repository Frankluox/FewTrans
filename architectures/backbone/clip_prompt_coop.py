...
# 和clip_prompt不同点：PromptLearner、custom clip、create model
...
class PromptLearner(nn.Module):
    def __init__(self, img_sz, classnameses, clip_model, cocoop =False):
        super().__init__()
        self.cocoop = cocoop
        n_ctx = 16
        ...
        # 每个prompt word设为“X”,替代 a photo of a。但无用
        ...
        self.all_token_prefix = []
        self.all_token_suffix = []
        self.all_n_cls = []
        ...
        for i, classnames in enumerate(classnameses):
            ...
            self.register_buffer(f"token_suffix_{i}", embedding[:, 1 + n_ctx :, :])  # CLS, EOS

            self.all_token_prefix.append(getattr(self, f"token_prefix_{i}"))
            self.all_token_suffix.append(getattr(self, f"token_suffix_{i}"))
            self.all_n_cls.append(n_cls)
    ...
    def forward(self, dataset_idx = 0, im_features = None):
        ctx = self.ctx
        if not self.cocoop and ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(self.all_n_cls[dataset_idx], -1, -1)


        prefix = self.all_token_prefix[dataset_idx].cuda()
        suffix = self.all_token_suffix[dataset_idx].cuda()
        ...
        if self.cocoop:
            ...
            for ctx_shifted_i in ctx_shifted:
                ctx_i = ctx_shifted_i.unsqueeze(0).expand(self.all_n_cls[dataset_idx], -1, -1)
...
class FixedEmbeddings():
    ...
    def return_fixed_embeddings(self, dataset_idx):
        return self.fixed_embeddings[dataset_idx]
    
    def return_specified_embeddings(self, dataset_idx, class_ids):
        specified_embeddings = torch.stack([self.fixed_embeddings[dataset_idx][i] for i in class_ids])
        return specified_embeddings

class CustomCLIP(nn.Module):
    def __init__(self, img_sz, all_classnames, clip_model, cocoop = False, ProGrad = False, kgcoop = False):
        super().__init__()
        self.cocoop = cocoop
        if ProGrad or kgcoop:
            self.embeddings = FixedEmbeddings(img_sz, all_classnames, clip_model)
        self.prompt_learner = PromptLearner(img_sz, all_classnames, clip_model, cocoop)
        self.tokenized_prompts = self.prompt_learner.all_tokenized_prompts
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype

    def forward(self, image, label = None, dataset_idx = 0, training = False, class_ids = None, prograd = False, kgcoop=False):
        logit_scale = self.logit_scale.exp()
        if self.cocoop:
            image_features = self.image_encoder(image.type(self.dtype))
            image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            prompts = self.prompt_learner(dataset_idx, image_features)
            # print(prompts.shape)
            # print(image_features.shape)
            prompts = torch.stack([prompts[:, i] for i in class_ids], dim=1)
            # print(prompts.shape)
            tokenized_prompts = self.tokenized_prompts[dataset_idx]
            ...
            if prograd:
                # print('hh')
                text_features = self.embeddings.return_specified_embeddings(dataset_idx, class_ids).cuda()
            else:
                prompts = self.prompt_learner(dataset_idx)
                tokenized_prompts = self.tokenized_prompts[dataset_idx]
                # print(prompts.shape)
                # print(tokenized_prompts.shape)
                prompts = torch.stack([prompts[i] for i in class_ids])
                tokenized_prompts = torch.stack([tokenized_prompts[i] for i in class_ids])
                text_features = self.text_encoder(prompts, tokenized_prompts)
                if kgcoop:
                    text_features_old = self.embeddings.return_specified_embeddings(dataset_idx, class_ids).cuda()
                    text_features_old = text_features_old / text_features_old.norm(dim=-1, keepdim=True)

            
            # text_features = torch.stack([text_features[i] for i in class_ids])
            ...
            if kgcoop:
                ...
                return F.cross_entropy(logits, label)+score
...
def create_model(all_classnames, mode = "coOp", backbone_name="ViT-B/16", img_sz=224):
# def create_model(all_classnames, backbone_name="ViT-B/16", prompt_depth_vision=0, n_ctx_vision=8, img_sz=224):
    ...
    if mode == "coOp" or mode == "ProGrad" or mode == "kgcoop":
        if mode == "ProGrad":
            model = CustomCLIP(img_sz, all_classnames, clip_model,False, True)
        else:
            if mode == "kgcoop":
                model = CustomCLIP(img_sz, all_classnames, clip_model,False, kgcoop=True)
            else:
                model = CustomCLIP(img_sz, all_classnames, clip_model,False)
    else:
        model = CustomCLIP(img_sz, all_classnames, clip_model,True)
