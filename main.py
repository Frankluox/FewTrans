...
def test(config):
    test_dataloader, test_dataset = create_torch_dataloader(config)
    print(config.DATA.TEST.DATASET_NAMES[0])

    logger.info(f"Creating model:{config.MODEL.TYPE}/{config.MODEL.NAME}")

    if "clip_prompt" in config.MODEL.BACKBONE:
        dataset_specs = test_dataset.dataset_specs
    else:
        dataset_specs = None
    ...
    with torch.no_grad():
        results, all_accs = testing(config, test_dataset, test_dataloader, model)
    for key, value in results.items():
        logger.info(f"Test Accuracy of {key}: {value[0]:.2f}%+-{value[1]:.2f}")
    ...
    # for key, value in results.items():
    #     if key == "base" or key == "novel":
    #         result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"][key] = [value[0], value[1]]
    #     else:
    #         result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"][config.DATA.TEST.DATASET_NAMES[key]] = [value[0], value[1]]

    # 修改为支持每个超参数组合的独立准确率记录
    for key, value in results.items():
        # 如果 key 是字符串（包括 "base", "novel" 以及我们新加的带超参数描述的 key），直接作为字典的键
        if isinstance(key, str):
            result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"][key] = [value[0], value[1]]
        else:
            # 原始逻辑：如果 key 是整数索引，则通过索引查找数据集名称
            # 这通常用于处理 Meta-Dataset 中的多任务/多数据集索引
            dataset_name = config.DATA.TEST.DATASET_NAMES[key]
            result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"][dataset_name] = [value[0], value[1]]

    with open(path, 'w') as f:
        json.dump(result_dic, f)

    # 为了做显著性检验，需要导出每个任务的原始准确率。
    raw_acc_path = os.path.join(config.OUTPUT, "raw_task_accs.json")
    dataset_name = config.DATA.TEST.DATASET_NAMES[0] # 获取当前数据集名称

    # 加载已有的原始数据（如果存在），防止多数据集测试时被覆盖
    if os.path.exists(raw_acc_path):
        with open(raw_acc_path, 'r') as f:
            raw_results_all = json.load(f)
    else:
        raw_results_all = {}

    # 将当前数据集的各任务准确率存入对应的 Key 下
    raw_results_all[dataset_name] = {key: [a.item() for a in acc_list] for key, acc_list in all_accs.items()}
    
    with open(raw_acc_path, 'w') as f:
        json.dump(raw_results_all, f)
    logger.info(f"Raw task-level accuracies for {dataset_name} saved to {raw_acc_path}")
...
def train_one_epoch(config, model, dataset, data_loader, optimizer, epoch, lr_scheduler, step, writer):
    ...
    for idx, batches in enumerate(data_loader):
        loss, acc = model.train_forward(*batches)
...
def validate(config, dataset, data_loader, model, epoch=None, writer=None):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        loss, acc = model.val_forward(*batches)
...
def testing(config, dataset,data_loader, model):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        for key, value in acc.items():
            all_accs[key].extend(value)
            # print(len(all_accs[key]))
        # print(list(acc.keys()))
        if "base" in acc:
            acc_log = torch.mean(torch.stack(acc["base"]))
            name = "base"
        elif "novel" in acc:
            acc_log = torch.mean(torch.stack(acc["novel"]))
            name = "novel"

        acc_meter.update(acc_log.item())
        ...
        if idx % config.PRINT_FREQ == 0:
            logger.info(
                ...
                f'Time: {batch_time.val:.2f} ({batch_time.avg:.2f})\t'
                f'Acc@{name}: {acc_meter.avg:.2f}\t')
    results = {}
    for key, acc in all_accs.items():
        acc = torch.stack(acc)
        results[key] = [torch.mean(acc).item(), (1.96*torch.std(acc)/math.sqrt(acc.shape[0])).item()]
    # return results
